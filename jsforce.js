  ...
  connection: async function (idProyecto) {
    ...
    try {
      await conn.login(
        ...
        process.env.password + process.env.securityToken
      ); //cambiar a ouath y variable de ambiente
      ...
      let soqlProyecto =
        "SELECT 	Banco__c,Razon_social__c,Numero_de_cuenta_pesos__c,alias_pesos__c,cbu_pesos__c, Cuit__c, numero_de_cuenta_dolares__c,cbu_dolares__c,alias_dolares__c FROM obra__c WHERE id = '" +
        idProyecto +
        "'LIMIT 1";
      var proyecto = await conn.query(soqlProyecto);
      let soqlUnidad =
        "SELECT id FROM unidad__c WHERE obra__c = '" + idProyecto + "'";
      ...
      let soqlEstados =
        ...
        "AND mes__c = " +
        month +
        " AND Email_enviado__c = false LIMIT 200";
